...
# Interactive script to set environment variables and update .zshrc

# Add a comment and empty line at the beginning of .zshrc
echo -e "\n# env vars for isucon" >> ~/.zshrc
...
# Prompt for APP_DIR and update .zshrc
...
# Prompt for APP_BUILD_CMD and update .zshrc
default_app_build_cmd="make"
app_build_cmd=$(prompt_for_input "Enter the value for APP_BUILD_CMD" "$default_app_build_cmd")
echo "export APP_BUILD_CMD=\"$app_build_cmd\"" >> ~/.zshrc

# Prompt for APP_SERVICE_NAME and update .zshrc
...
echo "export APP_SERVICE_NAME=\"$app_service_name\"" >> ~/.zshrc

# Prompt for SLACK_TOKEN and update .zshrc
...
echo "export SLACK_TOKEN=\"$slack_token\"" >> ~/.zshrc
